...
const ArtworkCardSkeleton = () => (
  <div className="rounded-2xl overflow-hidden animate-pulse bg-primary border border-primary-line w-full sm:w-[48%] lg:w-[31%]">
...
const EditArtworkModal = ({
  ...
  return (
    <div className="w-full rounded-2xl overflow-hidden">
      ...
      <div
        className="px-4 sm:px-6 py-4 flex flex-col sm:flex-row justify-end gap-3"
        style={{ borderTop: "1px solid #1a2e1a" }}
      >
        <button
          onClick={onClose}
          className="px-5 py-2 rounded-xl text-sm font-bold text-zinc-500 w-full sm:w-auto"
        >
          Cancel
        </button>
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="px-8 py-2 rounded-xl text-sm font-black flex items-center justify-center gap-2 w-full sm:w-auto"
          style={{
            background:
...
const ArtworkCard = ({
  ...
  return (
    <div
      className="group w-full sm:w-[48%] lg:w-[31%] rounded-2xl overflow-hidden transition-all border border-primary-line cursor-pointer bg-primary duration-300 hover:translate-y-[-2px]"
      onMouseEnter={(e) =>
    ...
    >
      <div className="relative h-52 overflow-hidden">
        <Link
          href={`/explore/${artwork.artwork_id}`}
...
const Artworks = () => {
  ...
  const [editArtwork, setEditArtwork] =
    useState<any | null>(null);

  const isMyAccount =
    !!userDetails?.username &&
    userDetails.username.replace(/^@/, "") ===
      profileId;
      ...
      try {
        const [profileRes, artworkRes] =
          await Promise.all([
            ...
            fetch(
              `/api/artwork/get/by-username?username=${profileId}`
            ),
          ]);
  ...
  const handleEditSave = (updated: any) => {
    setArtworks((prev) =>
      prev.map((a) =>
        a.artwork_id === updated.artwork_id
          ? updated
          : a
      )
    );
  ...
  return (
    <div className="px-1 max-w-[1280px] w-full m-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        ...
        {isMyAccount && (
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-sm font-black transition-all hover:scale-105"
            style={{
              background:
                "linear-gradient(135deg,#16a34a,#22c55e)",
              color: "#000",
      ...
      {isLoading ? (
        ...
        <div
          className="flex flex-col items-center justify-center py-20 sm:py-24 rounded-2xl px-4 text-center"
          style={{
        ...
        >
          ...
          >
            {isMyAccount
              ? "Upload your first piece to start selling"
              : "Check back later"}
          </p>
          ...
          {isMyAccount && (
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-black w-full sm:w-auto justify-center"
              style={{
                background:
                  "linear-gradient(135deg,#16a34a,#22c55e)",
                color: "#000",
      ...
      <Modal
        ...
        className="w-full max-w-[700px] p-2 sm:p-4"
      >
        <CreateArtwork
          onClose={() =>
            setIsCreateOpen(false)
      ...
      <Modal
        ...
        className="w-full max-w-[600px] p-2 sm:p-4"
      >
        {editArtwork && (
          <EditArtworkModal
            artwork={editArtwork}
